...
class ProvinceRepository {
  constructor () {
    ...
    this._limit = 5;
    this._number = 0;
    this._searchFields = [this._primaryKey, 'province_name', 'created_date'];
  }
  ...
  async getOne (id = '') {
    if (id === '') throw new BadRequestError('ID Province Required');

    const data = await this._model.findOne({ where: { unique_id: id } });

    if (!data) throw new NotFoundError('Province not found');

    return data;
  }

  async add (userId, params) {
    const { province_name: provinceName, screen_id: screenId } = params;

    const checkDuplicate = await this.checkDuplicate(provinceName);

    if (checkDuplicate >= 1) throw new ConflictError(`${provinceName} already Created`);
    ...
    try {
      ...
      throw new UnprocessableEntityError('Add Province Failed');
    }
  }

  async checkDuplicate (provinceName) {
  ...
  async checkDuplicateEdit (id, provinceName) {
    const check = await this._model.findAll({
      where: {
        province_name: provinceName,
        unique_id: {
  ...
  async update (id, userId, params) {
    ...
    const { province_name: provinceName } = params;

    const checkDuplicate = await this.checkDuplicateEdit(id, provinceName);

    if (checkDuplicate >= 1) throw new ConflictError(`${provinceName} alrady Created`);
    ...
    try {
      ...
      throw new UnprocessableEntityError('Update Province Failed');
    }
  }
  ...
  async delete (id, userId) {
    ...
    try {
      ...
      throw new UnprocessableEntityError('Delete Province Failed');
    }
  }
}
