...
class BusinessLineRepository {
  ...
  async add (userId, params) {
    ...
    const checkDuplicate = await this.checkDuplicate(businessLineName);

    if (checkDuplicate >= 1) throw new ConflictError(`${businessLineName} already Created`);
  ...
  async update (id, userId, params) {
    ...
    const checkDuplicate = await this.checkDuplicateEdit(id, businessLineName);
