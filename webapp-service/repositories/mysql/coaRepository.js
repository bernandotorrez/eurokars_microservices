const { Coa, sequelize } = require('../../models');
...
class CoaRepository {
  ...
  async getOne (id = '') {
    if (id === '') throw new BadRequestError('ID COA Required');

    const data = await this._model.findOne({ where: { unique_id: id } });

    if (!data) throw new NotFoundError('COA not found');
  ...
  async add (userId, params) {
    ...
    try {
      ...
      const [results] = await sequelize.query(
        'CALL sp_add_ms_coa(:userId, :coaCode, :coaName, :coaDescription, :screenId, :uniqueId);',
        {
          replacements: { userId, coaCode, coaName, coaDescription, screenId, uniqueId },
      ...
      if (results) {
        ...
        throw new UnprocessableEntityError('Add COA Failed');
      }
    } catch (error) {
  ...
  async checkDuplicate (coaName, coaCode) {
    const check = await this._model.findAll({
			where: {
				[Op.or]: [
					{ coa_name: coaName },
					{ coa_code: coaCode.toUpperCase() },
				],
			}
    });
  ...
  async checkDuplicateEdit (id, coaName, coaCode) {
    const check = await this._model.findAll({
      where: {
        [Op.or]: [
          { coa_name: coaName },
  ...
  async update (id, userId, params) {
    ...
    try {
      // Call the stored procedure
      const [results] = await sequelize.query(
        'CALL sp_update_ms_coa(:userId, :coaCode, :coaName, :coaDescription, :uniqueId);',
        {
          replacements: { userId, coaCode: coaCode.toUpperCase(), coaName, coaDescription, uniqueId: id },
          type: sequelize.QueryTypes.RAW
        }
      );
      ...
      if (results) {
        ...
        throw new UnprocessableEntityError('Update COA Failed');
      }
    } catch (error) {
      // Re-throw custom errors
      if (error instanceof ConflictError ||
  ...
  async delete (id, userId) {
    ...
    try {
      ...
      throw new UnprocessableEntityError('Delete COA Failed');
    }
  }
}

module.exports = new CoaRepository();
