...
class TaxRepository {
  ...
  async getOne (id = '') {
    if (id === '') throw new BadRequestError('ID Tax Required');

    const data = await this._model.findOne({ where: { unique_id: id } });

    if (!data) throw new NotFoundError('Tax not found');
  ...
  async add (userId, params) {
    ...
    try {
      ...
      const [results] = await sequelize.query(
        'CALL sp_add_ms_tax(:userId, :taxDescription, :taxCode, :screenId, :uniqueId);',
        {
          replacements: { userId, taxDescription, taxCode: taxCode.toUpperCase(), screenId, uniqueId },
      ...
      if (results) {
        ...
        throw new UnprocessableEntityError('Add Tax Failed');
      }
    } catch (error) {
  ...
  async checkDuplicate (taxDescription, taxCode) {
  ...
  async checkDuplicateEdit (id, taxDescription, taxCode) {
    const check = await this._model.findAll({
      where: {
        [Op.or]: [
          { tax_description: taxDescription },
  ...
  async update (id, userId, params) {
    ...
    try {
      // Call the stored procedure
      const [results] = await sequelize.query(
        'CALL sp_update_ms_tax(:userId, :taxDescription, :taxCode, :uniqueId);',
        {
          replacements: { userId, taxDescription, taxCode: taxCode.toUpperCase(), uniqueId: id },
          type: sequelize.QueryTypes.RAW
        }
      );
      ...
      if (results) {
        ...
        throw new UnprocessableEntityError('Update Tax Failed');
      }
    } catch (error) {
      // Re-throw custom errors
      if (error instanceof ConflictError ||
  ...
  async delete (id, userId) {
    ...
    try {
      ...
      throw new UnprocessableEntityError('Delete Tax Failed');
    }
  }
}
