...
class TaxRepository {
  ...
  async add (userId, params) {
    ...
    const checkDuplicate = await this.checkDuplicate(taxDescription, taxCode.toUpperCase());

    if (checkDuplicate >= 1) throw new ConflictError(`${taxDescription} or ${taxCode.toUpperCase()} already Created`);
  ...
  async update (id, userId, params) {
    ...
    const checkDuplicate = await this.checkDuplicateEdit(id, taxDescription, taxCode.toUpperCase());
