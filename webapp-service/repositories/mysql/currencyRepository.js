...
class CurrencyRepository {
  ...
  async add (userId, params) {
    ...
    try {
      ...
      const [results] = await sequelize.query(
        'CALL sp_add_ms_currency(:userId, :currencyName, :currencyCode, :screenId, :uniqueId);',
        {
          replacements: { userId, currencyName, currencyCode: currencyCode.toUpperCase(), screenId, uniqueId },
  ...
  async update (id, userId, params) {
    ...
    try {
      // Call the stored procedure
      const [results] = await sequelize.query(
        'CALL sp_update_ms_currency(:userId, :currencyName, :currencyCode, :uniqueId);',
        {
          replacements: { userId, currencyName, currencyCode: currencyCode.toUpperCase(), uniqueId: id },
          type: sequelize.QueryTypes.RAW
        }
      );
